...
$g.engineDir = phantom.casperScriptBaseDir;
$g.md5 = require($g.engineDir + '/md5.js').md5;
$g.childProcess = require('child_process');
$g.system = require('system');
$g.utils = require('utils');
$g.common = require($g.engineDir + '/common.js').create($g);
$g.taskPath = $g.pwd;
...
var requests = [], reqHeaders = [], responses = [], calcNetworkSize = false;
function abortRequest(requestData, networkRequest) {
    ...
    if (taskConfig.ignoreSinaIm && (requestData.url.indexOf('.weibo.com/im/') != -1 ||
        requestData.url.indexOf('/js/webim.js?') != -1)) {
        networkRequest.abort();
        return true;
    }
    if (!taskConfig.needImageData && requestData.url.startsWith('data:')) {
        networkRequest.abort();
        return true;
    }
    if (taskConfig.ignoreBaidu && (requestData.url.indexOf('cb.baidu.com') != -1 ||
        requestData.url.indexOf('cbjs.baidu.com') != -1 ||
        requestData.url.indexOf('cpro.baidustatic.com') != -1 ||
        requestData.url.indexOf('dup.baidustatic.com') != -1)) {
        networkRequest.abort();
        return true;
    }
    if (taskConfig.ignoreGoogle && (requestData.url.indexOf('googleads.g.doubleclick.net/pagead/ads?') != -1 ||
        requestData.url.indexOf('googleads.g.doubleclick.net/pagead/') != -1 ||
        requestData.url.indexOf('partner.googleadservices.com') != -1 ||
        requestData.url.indexOf('pagead2.googlesyndication.com') != -1 ||
        requestData.url.indexOf('tpc.googlesyndication.com') != -1 ||
        requestData.url.indexOf('www.googletagservices.com') != -1 ||
...
function createCasper() {
    ...
    casper.on('page.initialized', function (page) {
        page.evaluate(function (useEnglish) {
            (function() {
                var fnavigator = {};
...
$g.casper = casper;
$g.saveNid = function(data) {
    $g.fs.write($g.getPath("nid.txt"), JSON.stringify(data));
}
...
$g.findPage = 0;
require($g.engineDir + "/actions.js").create(casper, $g);
...
function analyzeLabels() {
    ...
    for (var i = 0, j = 0, cnt = taskConfig.actions.length; i < cnt; ++i) {
        ...
        if (actionInfo.name == 'loop') {
            ...
            if (actionInfo.variables && $g.utils.isString(actionInfo.variables)) {
...
function createActions() {
    result = TASK_FAILED;
    $g.trimFaceImg = false;
    if (taskConfig.trimFaceImg) $g.trimFaceImg = true;
    ...
    if (taskConfig.debugRequest) debugRequest = true;
    ...
    if ($g.variables.homeUrl.length > 0) {
        ...
        if (taskConfig.referer) {
            ...
            //$g.utils.dump(taskConfig.referer);
        }
    }
    ...
    if ($g.variables.homeUrl.length === 0) actionDelta -= 1;
    ...
    if (!taskConfig.loginSteps && taskConfig.checkers && taskConfig.checkers.logined) {

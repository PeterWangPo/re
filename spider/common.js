...
Common.create = function(_$g) {
    ...
    $g.$json = function(filePath) {
        ...
        var data = false;
        $g.$nothrow(function() {
            data = JSON.parse($g.fs.read(filePath));
        });
        return data;
    }
    ...
    $g.$trim = function(str, aTrim) {
        ...
        if (aTrim == 'zeroWhenEmpty') {
            return $g.isEmpty(str) ? '0' : str;
        } else if (aTrim == "digit") {
            var re = /(\d+)/g;
            if (re.test(str)) {
                str = RegExp.$1;
        ...
        } else if (aTrim.startsWith("beforeL:")) {
            ...
            if (rpl.length > 0) {
                var digitStart = parseInt(rpl);
                if (!isNaN(digitStart)) {
                    return str.substring(digitStart);
                } else {
                    var pos = str.lastIndexOf(rpl);
                    if (pos != -1) {
        ...
        } else if (aTrim.startsWith("afterF:")) {
            var rpl = aTrim.substring(7);
            if (rpl.length > 0) {
                var digitStart = parseInt(rpl);
                if (!isNaN(digitStart)) {
        ...
        } else if (aTrim.startsWith("divide:")) {
            var re = /^\d+$/g;
            if (re.test(str)) {
                var rpl = aTrim.substring(7);
                if (rpl.length > 0) {
                    return parseInt(parseInt(str) / parseInt(rpl));
                }
            }
            return str;
            ...
            if (args.length == 3) {
                ...
                if (!isNaN(digitStart)) {
                    ...
                    return str.substring(digitStart, end);
                } else {
        ...
        } else if (aTrim.startsWith("betweenF:")) {
            var args = aTrim.split(":");
            if (args.length > 3) {
                args = [];
                var re = /'(.+)':'(.+)'/g;
            ...
            if (args.length == 3) {
                ...
                if (!isNaN(digitStart)) {
                    ...
                    var pos = str.indexOf(args[1]);
                    if (pos != -1) {
                        var pos2 = str.indexOf(args[2], pos + args[1].length);
                        if (pos2 != -1) {
                            return str.substring(pos + args[1].length, pos2);
                        }
                    }
        ...
        } else if (aTrim.startsWith("replace:")) {
            var args = aTrim.split(":");
            if (args.length == 3) {
                return str.replaceAll(args[1], args[2]);
            }
        } else if (aTrim == "removeSpaces") {
            return $g.trimAllSpaces(str);
        } else if (aTrim.startsWith("remove:")) {
            var rpl = aTrim.substring(7);
            return $g.replaceAll(str, rpl, "");
        } else if (aTrim == "removeScript") {
            str.replace(/<script>.*<\/script>/gm, "");
        } else {
            return str;
        }
        return '';

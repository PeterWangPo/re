...
function _parseCheckers(checkers, op) {
    ...
    for (var i = 0, cnt = checkers.length; i < cnt; ++i) {
        ...
        var tmp = false;
        if ($g.utils.isString(checker.group)) {
...
function crawlerFromUrl(item) {
    var url = casper.getCurrentUrl(), params = [], aItem = {};
    var pos = url.indexOf('?');
    if (pos != -1) {
...
function initActions() {
    ...
    casper._actions = {
            ...
            if (!params.interval) {
                if (params.popup) {
                    ...
                    casper.waitFor(function() {
                        ...
                        if (ret && $g.utils.isNumber(params.sucJump)){
        ...
        dumpElement:function(params) {
            casper.then(function() {
                $g.acts.update(params);
                $g.debug(casper.getElementInfo(params.selector));
                $g.debug(casper.getElementBounds(params.selector));
                if (params.toFile) {
                    $g.fs.write(JSON.stringify(casper.getElementInfo(params.selector)), $g.getPath(params.toFile));
                }
            });
        },
